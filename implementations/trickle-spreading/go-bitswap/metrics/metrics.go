...
func DupHist(ctx context.Context) metrics.Histogram {
	return metrics.NewCtx(ctx, "recv_dup_blocks_bytes", "Summary of duplicate data blocks recived").
		Histogram(metricsBuckets)
}

func AllHist(ctx context.Context) metrics.Histogram {
	return metrics.NewCtx(ctx, "recv_all_blocks_bytes", "Summary of all data blocks recived").
		Histogram(metricsBuckets)
}

func SentHist(ctx context.Context) metrics.Histogram {
	return metrics.NewCtx(ctx, "sent_all_blocks_bytes", "Histogram of blocks sent by this bitswap").
		Histogram(metricsBuckets)
}

func SendTimeHist(ctx context.Context) metrics.Histogram {
	return metrics.NewCtx(ctx, "send_times", "Histogram of how long it takes to send messages in this bitswap").
...
func PendingBlocksGauge(ctx context.Context) metrics.Gauge {
	return metrics.NewCtx(ctx, "pending_block_tasks", "Total number of pending blockstore tasks").
		Gauge()
}

func ActiveBlocksGauge(ctx context.Context) metrics.Gauge {
	return metrics.NewCtx(ctx, "active_block_tasks", "Total number of active blockstore tasks").
		Gauge()
}
