...
func TestPushSizeInfo(t *testing.T) {
	...
	runTestCase := func(tasks []peertask.Task, expSize int, expBlockSize int, expIsWantBlock bool) {
		...
		if td.IsWantBlock != expIsWantBlock {
			t.Fatalf(
				"Expected task.IsWantBlock to be %t, received %t",
				expIsWantBlock,
				td.IsWantBlock,
			)
		}
	...
	runTestCase(
		[]peertask.Task{wantBlockDontHave, wantBlockDontHave},
		wantBlockDontHave.Work,
		wantBlockDontHaveBlockSize,
		isWantBlock,
	)
	// want-have (DONT_HAVE) should have no effect on existing want-block (DONT_HAVE)
	runTestCase(
		[]peertask.Task{wantBlockDontHave, wantHaveDontHave},
		wantBlockDontHave.Work,
		wantBlockDontHaveBlockSize,
		isWantBlock,
	)
	...
	runTestCase(
		[]peertask.Task{wantBlockDontHave, wantBlock},
		wantBlock.Work,
		wantBlockBlockSize,
		isWantBlock,
	)
	...
	runTestCase(
		[]peertask.Task{wantBlockDontHave, wantHave},
		wantHaveBlockSize,
		wantHaveBlockSize,
		isWantBlock,
	)
	...
	runTestCase(
		[]peertask.Task{wantBlock, wantBlockDontHave},
		wantBlock.Work,
		wantBlockBlockSize,
		isWantBlock,
	)
	...
	runTestCase(
		[]peertask.Task{wantBlock, wantHaveDontHave},
		wantBlock.Work,
		wantBlockBlockSize,
		isWantBlock,
	)
	...
	runTestCase(
		[]peertask.Task{wantBlock, wantBlock},
		wantBlock.Work,
		wantBlockBlockSize,
		isWantBlock,
	)
	...
	runTestCase(
		[]peertask.Task{wantBlock, wantHave},
		wantBlock.Work,
		wantBlockBlockSize,
		isWantBlock,
	)
	...
	runTestCase(
		[]peertask.Task{wantHaveDontHave, wantHaveDontHave},
		wantHaveDontHave.Work,
		wantHaveDontHaveBlockSize,
		isWantHave,
	)
	// want-block with size should update existing want-have (DONT_HAVE)
	runTestCase(
		[]peertask.Task{wantHaveDontHave, wantBlock},
		wantBlock.Work,
		wantBlockBlockSize,
		isWantBlock,
	)
	...
	runTestCase(
		[]peertask.Task{wantHaveDontHave, wantHave},
		wantHave.Work,
		wantHaveBlockSize,
		isWantHave,
	)
	...
	runTestCase(
		[]peertask.Task{wantHave, wantBlockDontHave},
		wantHaveBlockSize,
		wantHaveBlockSize,
		isWantBlock,
	)
	...
	runTestCase(
		[]peertask.Task{wantHave, wantHaveDontHave},
		wantHave.Work,
		wantHaveBlockSize,
		isWantHave,
	)
	...
	runTestCase(
		[]peertask.Task{wantHave, wantBlock},
		wantBlock.Work,
		wantBlockBlockSize,
		isWantBlock,
	)
...
func TestPushSizeInfoActive(t *testing.T) {
	...
	runTestCase := func(tasks []peertask.Task, expTasks []peertask.Task) {
		...
		for i, task := range popped {
			...
			if td.IsWantBlock != expTd.IsWantBlock {
				t.Fatalf(
					"Expected IsWantBlock to be %t, received %t",
					expTd.IsWantBlock,
					td.IsWantBlock,
				)
			}
	...
	runTestCase(
		[]peertask.Task{wantBlockDontHave, wantBlockDontHave},
		[]peertask.Task{wantBlockDontHave},
	)
	...
	runTestCase(
		[]peertask.Task{wantHaveDontHave, wantHaveDontHave},
		[]peertask.Task{wantHaveDontHave},
	)
