...
func (e *Engine) SetSessionManager(sm *bssm.SessionManager) {
	e.sm = sm
}
...
func newEngine(
	...
	e := &Engine{
		...
		sm:                              nil,
		relaySession:                    rs.NewRelaySession(),
	}
	...
	e.bsm = newBlockstoreManager(
		bs,
		e.bstoreWorkerCount,
		bmetrics.PendingBlocksGauge(ctx),
		bmetrics.ActiveBlocksGauge(ctx),
	)
	...
	if e.taskComparator != nil {
		...
		peerTaskQueueOpts = append(
			peerTaskQueueOpts,
			peertaskqueue.PeerComparator(
				peertracker.TaskPriorityPeerComparator(queueTaskComparator),
			),
		)
		peerTaskQueueOpts = append(
			peerTaskQueueOpts,
			peertaskqueue.TaskComparator(queueTaskComparator),
		)
	}
...
func (e *Engine) MessageReceived(ctx context.Context, p peer.ID, m bsmsg.BitSwapMessage) {
	...
	for _, entry := range cancels {
		...
		if e.relaySession.Session != nil {
			log.Debugf(
				"local: %v Removing key from received cancels in relay session registry",
				e.self,
			)
	...
	for _, entry := range denials {
		log.Debugw(
			"Bitswap engine: block denied access",
			"local",
			e.self,
			"from",
			p,
	...
	for _, entry := range wants {
		...
		if !found {
			log.Debugw(
				"Bitswap engine: block not found",
				"local",
				e.self,
				"from",
				p,
			...
			log.Debugf(
				"Updating tracker to start a new relay session for %s, %d",
				entry.Cid,
				entry.Priority,
			)
...
func (e *Engine) splitWantsDenials(
	p peer.ID,
	allWants []bsmsg.Entry,
) ([]bsmsg.Entry, []bsmsg.Entry) {
	...
	for _, h := range haves {
		...
		for ip := range interestedPeers {
			...
			e.peerRequestQueue.PushTasks(ip, peertask.Task{
				...
				Work:     entrySize,
				Data: &taskData{
					// BlockSize:    0, // Not sending a block, forwarding a have message. So don't have size.
					HaveBlock:    true,
