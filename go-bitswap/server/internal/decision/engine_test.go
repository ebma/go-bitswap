...
func newTestEngineWithSampling(
	ctx context.Context,
	idStr string,
	...
	e := newEngineForTesting(
		...
		append(
			...
			WithScoreLedger(NewTestScoreLedger(peerSampleInterval, sampleCh, clock)),
...
func TestConsistentAccounting(t *testing.T) {
	...
	if sender.Engine.numBytesSentTo(
		receiver.Peer,
	) != receiver.Engine.numBytesReceivedFrom(
		sender.Peer,
	) {
	...
	if receiver.Engine.numBytesSentTo(sender.Peer) != 0 ||
		sender.Engine.numBytesReceivedFrom(receiver.Peer) != 0 {
...
func TestOutboxClosedWhenEngineClosed(t *testing.T) {
	...
	e := newEngineForTesting(
		...
		"localhost",
		0,
		WithScoreLedger(NewTestScoreLedger(shortTerm, nil, clock.New())),
		WithBlockstoreWorkerCount(4),
	)
	e.StartWorkers(ctx, process.WithTeardown(func() error { return nil }))
...
func checkOutput(
	...
	if len(blks) != len(expBlks) {
		...
		msg := fmt.Sprintf(
			"Received %d blocks. Expected %d blocks:\n%s",
			len(blks),
			len(expBlks),
			blkDiff,
		)
...
func checkPresence(
	presences []message.BlockPresence,
	expPresence []string,
	presenceType pb.Message_BlockPresenceType,
) error {
...
func TestSendReceivedBlocksToPeersThatWantThem(t *testing.T) {
	...
	if len(sentHave) != 1 || !sentHave[0].Cid.Equals(blks[0].Cid()) ||
		sentHave[0].Type != pb.Message_Have {
...
func TestSendDontHave(t *testing.T) {
	...
	sentHave := env.Message.BlockPresences()
	if len(sentHave) != 2 || sentHave[0].Type != pb.Message_Have ||
		sentHave[1].Type != pb.Message_Have {
...
func TestWantlistForPeer(t *testing.T) {
	...
	if entries[0].Priority != 4 || entries[1].Priority != 3 || entries[2].Priority != 2 ||
...
func TestTaskComparator(t *testing.T) {
	...
	for i, peerID := range peerIDs {
		...
		if peerID != envelope.Peer {
			t.Errorf(
				"expected message for peer ID %#v but instead got message for peer ID %#v",
				peerID,
				envelope.Peer,
			)
		}
...
func checkHandledInOrder(t *testing.T, e *Engine, expected [][]string) error {
	for _, keys := range expected {
		...
		if len(received) != len(keys) {
			return errors.New(
				fmt.Sprintln("# blocks received", len(received), "# blocks expected", len(keys)),
			)
		}
		// Verify payload message contents
		for _, k := range keys {
			...
			if !found {
				return errors.New(
					fmt.Sprintln("received", received, "expected", string(expected.RawData())),
				)
			}
		}
	}
