...
func TestSessionBetweenPeers(t *testing.T) {
	...
	ig := testinstance.NewTestInstanceGenerator(
		vnet,
		nil,
		[]bitswap.Option{bitswap.SetSimulateDontHavesOnTimeout(false)},
	)
	defer ig.Close()
	bgen := blocksutil.NewBlockGenerator()
...
func TestFetchNotConnected(t *testing.T) {
	...
	ig := testinstance.NewTestInstanceGenerator(
		vnet,
		nil,
		[]bitswap.Option{bitswap.ProviderSearchDelay(10 * time.Millisecond)},
	)
	defer ig.Close()
	bgen := blocksutil.NewBlockGenerator()
